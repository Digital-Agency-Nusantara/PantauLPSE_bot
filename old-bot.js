  ...
  return {
    reply_markup: {
      keyboard: [
        ...
        ['🌐 URL Management', '⚙️ Settings'],
        ['🚀 Start Monitoring', '⏹️ Stop Monitoring'],
        ['📈 Status', '👤 Profile']
      ],
...
// URL Management Keyboard
function getUrlKeyboard() {
  return {
    reply_markup: {
      keyboard: [
        ['➕ Add URL', '📋 View URLs'],
        ['🗑️ Delete URL', '🏠 Back to Menu']
      ],
      resize_keyboard: true,
      one_time_keyboard: false
    }
...
bot.on('message', async (msg) => {
  ...
  switch (userStates[chatId]) {
    ...
    case 'awaiting_delete_keyword':
      ...
      break;
    case 'awaiting_delete_url':
      handleDeleteUrl(chatId, text);
      break;
    default:
...
function handleMenuSelection(chatId, text) {
  switch (text) {
    ...
    case '🌐 URL Management':
      bot.sendMessage(chatId, '🌐 URL Management Menu:', getUrlKeyboard());
      break;
    ...
    // URL Menu
    case '➕ Add URL':
      bot.sendMessage(chatId, 'Masukkan URL LPSE (contoh: https://lpse.jakarta.go.id):');
      userStates[chatId] = 'awaiting_url';
      break;
      
    case '📋 View URLs':
      handleViewUrls(chatId);
      break;
      
    case '🗑️ Delete URL':
      handleDeleteUrlMenu(chatId);
      break;
...
// URL handlers
function handleAddUrl(chatId, url) {
  try {
    new URL(url); // Validate URL
    if (!users[chatId].urlList.includes(url)) {
      users[chatId].urlList.push(url);
      saveData();
      bot.sendMessage(chatId, `✅ URL berhasil ditambahkan!`, getUrlKeyboard());
    } else {
      bot.sendMessage(chatId, `❌ URL sudah ada dalam daftar!`, getUrlKeyboard());
    }
  } catch (error) {
    bot.sendMessage(chatId, '❌ Format URL tidak valid!', getUrlKeyboard());
  }
  delete userStates[chatId];
}

function handleViewUrls(chatId) {
  const urlList = users[chatId].urlList;
  if (urlList.length === 0) {
    bot.sendMessage(chatId, '🌐 Daftar URL masih kosong.', getUrlKeyboard());
  } else {
    const message = '🌐 Daftar URL LPSE Anda:\n\n' + urlList.map((url, index) => `${index + 1}. ${url}`).join('\n');
    bot.sendMessage(chatId, message, getUrlKeyboard());
  }
}

function handleDeleteUrlMenu(chatId) {
  const urlList = users[chatId].urlList;
  if (urlList.length === 0) {
    bot.sendMessage(chatId, '🌐 Daftar URL masih kosong.', getUrlKeyboard());
  } else {
    const message = '🗑️ Pilih nomor URL yang akan dihapus:\n\n' + 
                   urlList.map((url, index) => `${index + 1}. ${url.substring(0, 50)}...`).join('\n') +
                   '\n\nKetik nomor urut (contoh: 1):';
    bot.sendMessage(chatId, message);
    userStates[chatId] = 'awaiting_delete_url';
  }
}

function handleDeleteUrl(chatId, text) {
  const index = parseInt(text) - 1;
  const urlList = users[chatId].urlList;
  
  if (index >= 0 && index < urlList.length) {
    const deletedUrl = urlList.splice(index, 1)[0];
    saveData();
    bot.sendMessage(chatId, `✅ URL berhasil dihapus!`, getUrlKeyboard());
  } else {
    bot.sendMessage(chatId, '❌ Nomor tidak valid!', getUrlKeyboard());
  }
  delete userStates[chatId];
}
...
function handleStatus(chatId) {
  ...
  const message = `📈 Status Bot:\n\n` +
                 ...
                 `• URLs: ${user.urlList.length} items\n` +
...
function handleProfile(chatId) {
  ...
  const message = `👤 Profile:\n\n` +
                 ...
                 `🌐 URLs Count: ${user.urlList.length}\n` +
                 `📢 Total Notifications: ${user.sentTenders.length}`;
  
  bot.sendMessage(chatId, message, getMainMenuKeyboard());
}

// Scraping functions (from original script)
...
async function startScrapingLoop() {
  ...
  while (isScrapingActive) {
    try {
      ...
      // Get all unique URLs from all users
      const allUrls = [...new Set(activeUsers.flatMap(user => user.urlList))];
...
bot.onText(/\/admin_stats/, (msg) => {
  ...
  const totalUrls = Object.values(users).reduce((sum, user) => sum + user.urlList.length, 0);
...
bot.onText(/\/admin_broadcast (.+)/, (msg, match) => {
  ...
  // You can add admin user IDs here
  const adminIds = ['YOUR_ADMIN_CHAT_ID']; // Replace with actual admin chat IDs
